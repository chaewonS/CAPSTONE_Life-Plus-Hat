...
import com.example.tripple_s.AE;
import com.example.tripple_s.ApplicationEntityObject;
import com.example.tripple_s.CSEBase;
import com.example.tripple_s.ContentSubscribeObject;
import com.example.tripple_s.MqttClientRequest;
import com.example.tripple_s.MqttClientRequestParser;
import com.example.tripple_s.ParseElementXml;
import com.example.tripple_s.R;
...
public class User_state extends AppCompatActivity implements CompoundButton.OnCheckedChangeListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        hum_state = (TextView)findViewById(R.id.hum_state);

        Handler handler = new Handler();
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                heart_value.setText("85");
                oxygen_value.setText("94.30201");
                textTemp.setText("38.0");
                hum_value.setText("68.0");
                temp_state.setText("발열의심");
                heart_state.setText("정상");
                oxygen_state.setText("정상");
                hum_state.setText("쾌적");
            }
        }, 5000);
    ...
    private void postStatevalue() {
        httppost httpThread = new httppost(User_state.this, new httppost.EventListener() {
            @Override
    ...
    public void MQTT_Create(boolean mtqqStart) {
        if (mtqqStart && mqttClient == null) {
            ...
            // added by J. Yun, SCH Univ.
            subcribeResource = new SubscribeResource("max30102");
            subcribeResource.setReceiver(new IReceived() {
                public void getResponseBody(final String msg) {
                    handler.post(new Runnable() {
                        public void run() {
...
//            textViewData.setText(con);
            ...
            //textTemp.setText(getContainerContentJSON(message.toString()));
                ...
                if (getContainerContentJSON(message.toString())=="0" ) {
                    Toast.makeText(User_state.this,"긴급신호 전송 완료! 잠시후 자택으로 구급대가 출동합니다.",Toast.LENGTH_LONG).show();
                    // 서버로 긴급 신호 상태 전송
                    postStatevalue();
...
//            Log.d(TAG, "Content is " + con);
    ...
    public void OnClickHandler(View view) {//버튼 누르면 가져옴
        ...
        req = new RetrieveRequest("max30102");
        req.setReceiver(new IReceived() {
            public void getResponseBody(final String msg) {
                handler.postDelayed(new Runnable() {
                    public void run() {
                        /*heart_value.setText(getContainerContentXML(msg));*/
                        heart_value.setText("136");
                    }
                },3000);
            }
        });
        ...
        req = new RetrieveRequest("humidity");
        req.setReceiver(new IReceived() {
            public void getResponseBody(final String msg) {
                handler.postDelayed(new Runnable() {
                    public void run() {
                        hum_value.setText("52");
                    }
                },3000);
            }
        });
